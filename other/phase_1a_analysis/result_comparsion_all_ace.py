...
JSON_FILE = 'master_timeslice_one_remainder_actor_filtered_protagonist'
...
for row in tqdm(twitter_data.itertuples(), total=len(twitter_data)):
    ...
    try:
        ...
        annotation_df = annotation_df.drop(["name", "id", "providerName"], axis=1)
        # print(annotation_df)
    except ValueError:
        count_Nan += 1
        print("Nan value {}!".format(count_Nan))
        continue
    else:
        # all
