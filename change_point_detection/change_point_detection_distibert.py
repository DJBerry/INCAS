...
for row in tqdm(sample_df.itertuples(), total=len(sample_df)):
    ...
    _, output = model(**encoding, return_dict=False)
    output_numpy = output.detach().numpy()
    output_numpy = output_numpy[0]
    features_raw.append({
        "text_vec": output_numpy
    })
features_df = pd.DataFrame(features_raw, index=sample_df.index)
tfidf_feature_df = pd.concat([sample_df, features_df], axis=1)
...
res_multi, res_multi_result = metachange.change_point_tree(vec_array, date_array, clf_rf, min_range=MIN_RANGE)

end_time = time.time()

# print("preoprocess time: {}".format(preprocess_time - start_time))
print("vectorizer time: {}".format(vectorizer_time - start_time))
